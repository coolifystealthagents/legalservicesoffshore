...
for (const e of manifest.entries) {
  ...
  assert.equal(e.provenance, 'repair-replacement');
  ...
  const before = execFileSync('git', ['show', `${e.introducedByCommit}^:app/fleet-data.ts`], { encoding: 'utf8' });
  const after = execFileSync('git', ['show', `${e.introducedByCommit}:app/fleet-data.ts`], { encoding: 'utf8' });
  assert.equal(before.includes(`'${e.slug}': '2026-08-10'`), false);
  assert.equal(after.includes(`'${e.slug}': '2026-08-10'`), true);
